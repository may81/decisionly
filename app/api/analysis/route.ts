...
function buildInsights(
  ...
  if (
    ...
    insights.push({
      ...
      message:
        `Assets do not equal liabilities plus equity. The current difference is ${latest.balanceDifference.toLocaleString(
          "id-ID"
        )}.`,
      priority: 100,
    });
  ...
  if (
    ...
    insights.push({
      ...
      message:
        "Operating activities are consuming cash. Review collections, working capital, and recurring operating costs.",
      priority: 90,
    });
  } else if (
    latest.operatingCashFlow > 0
...
async function getReportLimit(
  ...
  return data as ReportLimitResult;
}
